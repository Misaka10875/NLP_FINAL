...
class AblationDetector:
    def __init__(self, model_name, config, output_dir):
        ...
        self.output_file = os.path.join(output_dir, f'{model_name}_ablation_detection.csv')
    ...
    def process_row(self, row_data):
        ...
        ablation_content = row['ablation_rewritten_content']
        
        # 跳过改写错误的样本
        if pd.isna(ablation_content) or "ERROR:" in str(ablation_content):
            with self.lock:
                self.error_count += 1
                if self.pbar:
                    self.pbar.update(1)
        ...
        # 调用模型检测
        answer = self.call_model_detect(ablation_content)
    ...
    def calculate_asr(self, total_samples):
        # ASR = 改写后被误判为非诈骗的数量 / 总样本数
        asr = (self.detected_as_nonfraud / total_samples * 100) if total_samples > 0 else 0
        ...
        return {
            ...
            'ablation_asr_percent': round(asr, 2),
...
def load_ablation_rewritten_data(ablation_results_dir):
    print("正在加载消融改写后的数据...")
    
    ablation_data = {}
    
    for model_name in MODELS_CONFIG.keys():
        ablation_file = os.path.join(ablation_results_dir, f'{model_name}_ablation_rewritten.csv')
        
        if os.path.exists(ablation_file):
            df = pd.read_csv(ablation_file)
            # 过滤掉改写错误的样本
            df = df[~df['ablation_rewritten_content'].str.contains('ERROR:', na=False)]
            ablation_data[model_name] = df
            print(f"{model_name}: 加载消融改写数据 {len(df)} 条")
        else:
            print(f"警告: 未找到 {model_name} 的消融改写文件")
            ablation_data[model_name] = pd.DataFrame()
    
    return ablation_data

def main():
    # 加载消融改写后的数据
    ablation_results_dir = 'ablation_rewrite_results'
    ablation_data = load_ablation_rewritten_data(ablation_results_dir)
    
    # 创建输出目录
    output_dir = 'ablation_detection_results'
    os.makedirs(output_dir, exist_ok=True)
    
    # 创建检测器
    detectors = {}
    for name, config in MODELS_CONFIG.items():
        if name in ablation_data and len(ablation_data[name]) > 0:
            detectors[name] = AblationDetector(name, config, output_dir)
    
    if len(detectors) == 0:
        print("错误: 没有可用的检测器，请检查消融改写结果")
        return
    ...
    for name, detector in detectors.items():
        total_samples = len(ablation_data[name])
        asr_report = detector.calculate_asr(total_samples)
        asr_reports.append(asr_report)
        
        print(f"\n{'='*60}")
        print(f"模型: {name}")
        print(f"消融改写样本总数: {asr_report['total_samples']}")
        print(f"检测判为诈骗: {asr_report['detected_as_fraud']}")
        print(f"检测判为非诈骗: {asr_report['detected_as_nonfraud']}")
        print(f"消融实验ASR: {asr_report['ablation_asr_percent']}%")
    ...
    if asr_reports:
        ...
        asr_path = os.path.join(output_dir, 'ablation_asr_report.csv')
        asr_df.to_csv(asr_path, index=False, encoding='utf-8-sig')
        print(f"\n消融实验ASR报告已保存至: {asr_path}")
